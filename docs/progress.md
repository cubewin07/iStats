...
| Next task | `1.5` Implement the MetricsStore ring buffer |
...
| 1 | App foundation | **in progress** | App target, `LSUIElement`, `NSStatusItem` + detail popover, core protocols/types, and `SampleScheduler` complete; next: `MetricsStore`. |
...
|--------|-------------------------------|
| `Package.swift` → `iStatsCore` + `iStatsCoreTests`, `iStats.xcodeproj` (app target `iStats`), `iStatsApp`, `AppDelegate`, `MenuBarController`, `DetailPopoverView`, `DockIconManager`, `Info.plist` (`LSUIElement = true`), `NSStatusItem`, `NSPopover` | `MetricsStore`, preferences store |
| `Availability`, `Sample<T>`, `Sampler`, `SamplerError`, `MetricCategory`, `AnySampler`, `MetricReading`, `SampleScheduler` | Any concrete sampler (CPU, memory, …) |
...
| Tests: `RateMathTests`, `RingBufferTests`, `UnitsTests`, `AvailabilityTests`, `MetricCategoryTests`, `SamplerTests`, `ModelsTests`, `SampleSchedulerTests` | — |
