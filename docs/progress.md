...
| Next task | `1.4` implement the SampleScheduler |
...
| 1 | App foundation | **in progress** | App target, `LSUIElement`, `NSStatusItem` + detail popover, and core protocols/types complete; next: `SampleScheduler`. |
...
| `Availability`, `Sample<T>`, `Sampler`, `SamplerError`, `MetricCategory` | Any concrete sampler (CPU, memory, …) |
...
| Tests: `RateMathTests`, `RingBufferTests`, `UnitsTests`, `AvailabilityTests`, `MetricCategoryTests`, `SamplerTests`, `ModelsTests` | — |
